 ...
 *
 *	Author: James W. Beauchamp (jwb)
 *	Date: 1987
 *
 *	Changes: (zheng = Zheng (Geoffrey) Hua)
 *	11/9/88: install ref to axskip, allowing skip of axes plots
 *		 for monan specific plots
 *	07/03/89: add plotseg2()
 *	07/25/89: fix string out in plotbar()
 ...
 *	11/04/90: fix no. digits horiz. axis for large xinc
 *	11/04/90: extend range of if(!axskip) and make variables static
 *		  for plotseg() and plotseg1()
 *	11/06/90: insert research mode code (approve axis values)
 *		  including approve() routine
 *	02/28/91: replace HUGE with HUGEI for IEEE huge value given in
 *		  macro.h
 *	11/26/91: remove axskip, pause defs for non-monan applications.
 *		  remove research mode code and approve function.
 *	06/30/92: append with plotbar1, a different version of plotbar.
 *	07/10/92: install plabel in plotbar1()
 *	11/29/97  zheng	Use gets and sscanf for input
 *	10/09/98  jwb	plotseg2(): Add linethick function argument.
 *	01/29/14  jwb	For plotseg(), plotseg1(), plotseg3(), plotbar(),
 *			and plotbar1(), set line thickness to 10.
 *	11/02/17  jwb   cosmetic fixes
 *****************************************************************************/
#include <math.h>
#include <stdio.h>
#include "g_raph.h"
...
/*    P(stderr,"after floor/ceil: ymin=%f, ymax=%f, nvticks=%d\n",ymin,ymax,nvticks);
    ...
    g_init();
    g_set_line_thickness(10);				    /* jwb 01/29/14 */
/*
