...
void extendsyn(float** cmag, float** dfr, int nhar1, float length, float extension, float frameDuration)
{
    ...
    P("Apply low-pass filter to smooth original harmonic amplitudes\n\n");
    for (k = 1; k < nhar1; k++)
    {
        for (i = 0; i < npts; i++)
        {
        ...
        P("Choose the harmonic number for amplitude-vs-time plots: ");
        scanf("%d%*c", &kplot);				   /* jwb 11/17/17 */
        P("\nHit Return to plot the original harmonic amplitude curve: ");
        ...
        char svlabel[80];
        sprintf(svlabel,"HARMONIC AMPL %d", kplot);	    /* jwb 11/17/17 */
    ...
    P("\nNo. frames to compensate for LPF delay: %d\n", shiftamount);
    for (k = 1; k < nhar1; k++)
    {
    ...
    // printCSV("amplowpassed2.csv", "Low-pass (2st partial)", amplowpassed[2], npts, frameDuration);
        ...
        P("\nAfter smoothing and shift of harmonic amplitudes,\n");
        P("hit Return to plot the chosen harmonic amplitude: ");
        getchar();					    /* jwb 11/17/17 */
    ...
    for (k = 1; k < nhar1; k++)
    {
        for (i = 0 ; i < npts; i++)
        {
            ...
            if (amporiginal[k][i] == 0)
            {
                microvariations[k][i] = 0;
            }
            if (amplowpassed[k][i] == 0)
            {
                microvariations[k][i] = 1;
            }
        }
    }
        P("\nFor the original flattened microvariations,\n");
        P("hit Return to plot the chosen harmonic amplitude: ");
        getchar();
        ...
        sprintf(svlabel,"HARMONIC AMPL %d", kplot);
    ...
    for (k = 1; k < nhar1; k++)
    {
        ...
        P("\nAfter low pass and extend of harmonic amplitude,\n");
        P("hit Return to plot the chosen harmonic amplitude: ");
        getchar();
    ...
    *dfr = (float*)calloc(nptsnew * nhar1, sizeof(float));
    ...
    for (samplePointer = 0; samplePointer < decayf; samplePointer++)
    {
        for (k = 1; k < nhar1; k++)
        {
            ...
            else
            {
                ...
                // else
                // {
                //     amplitudeScale = amplowpassednew[k][samplePointer] / amplowpassed[k][samplePointer];
                // }
                (*cmag)[k + samplePointer * nhar1] = amplowpassednew[k][samplePointer] + amporiginal[k][samplePointer] - amplowpassed[k][samplePointer];
    ...
    while (counter > 0)
    {
        ...
        for (j = decayf; j > attackf; j--)
        {
            for (k = 1; k < nhar1; k++)
            {
                ...
                // }
                (*cmag)[k + samplePointer * nhar1] = amplowpassednew[k][samplePointer] + amporiginal[k][j] - amplowpassed[k][j];
    ...
    P("\nAfter linear extension of smoothed amplitude and addition ");
    P("of zig-zagged\n microvariations, ");		    /* jwb 11/17/17 */
    P("hit Return to plot final harmonic curve\n");	    /* jwb 11/17/17 */
    getchar();						    /* jwb 11/17/17 */
